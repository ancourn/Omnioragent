...
async function logStep(runPath: string, runId: string, step: string, status: string, note: string = "") {
  ...
  try {
    // Write to file system (existing functionality)
    let data: LogEntry[] = [];
    try {
    ...
    try {
      ...
      console.error('Error writing log to database:', dbError);
    }
  } catch (error) {
...
async function saveArtifact(runId: string, type: string, artifactPath: string, content?: string) {
  try {
...
async function runPipeline(nlGoal: string): Promise<{ runPath: string; runId: string }> {
  ...
  try {
    ...
    // Fallback to using timestamp as ID
    runId = `run-${timestamp}`;
  }
  
  await logStep(runPath, runId, "start", "ok", nlGoal);
  
  const plan = planner(nlGoal);
  await logStep(runPath, runId, "planner", "ok", JSON.stringify(plan));
  
  const spec = specifier(plan);
  const specPath = path.join(runPath, 'SPECIFIER.json');
  await fs.writeFile(specPath, JSON.stringify(spec, null, 2));
  await saveArtifact(runId, 'spec', specPath, JSON.stringify(spec, null, 2));
  await logStep(runPath, runId, "specifier", "ok", "spec saved");
  
  try {
    const coderSuccess = await coder(spec, runPath);
    await logStep(runPath, runId, "coder", "ok", "repo created");
    
    // Save code artifacts
    const repoPath = path.join(runPath, 'repo');
    await saveArtifact(runId, 'code', repoPath);
  } catch (error: any) {
    await logStep(runPath, runId, "coder", "fail", error.message);
    ...
    try {
      await db.run.update({
        ...
        data: { 
          ...
          summary: `Pipeline failed at coder step: ${error.message}`
        }
      });
    } catch (dbError) {
      console.error('Error updating run status:', dbError);
    }
  ...
  await logStep(runPath, runId, "tester", testOk ? "ok" : "fail", testNote);
  
  const [fixOk, fixNote] = await fixer(runPath, testOk);
  await logStep(runPath, runId, "fixer", fixOk ? "ok" : "fail", fixNote);
  
  const [deployOk, deployNote] = await deployer(runPath);
  await logStep(runPath, runId, "deployer", deployOk ? "ok" : "fail", deployNote);
  
  const evalRes = evaluator(runPath, testOk, deployOk);
  const evalPath = path.join(runPath, 'EVALUATION.json');
  await fs.writeFile(evalPath, JSON.stringify(evalRes, null, 2));
  await saveArtifact(runId, 'eval', evalPath, JSON.stringify(evalRes, null, 2));
  await logStep(runPath, runId, "evaluator", "ok", JSON.stringify(evalRes));
  
  await logStep(runPath, runId, "finish", "ok", "pipeline complete");
  ...
  try {
    await db.run.update({
      ...
      data: { 
        status: 'completed',
        finished_at: new Date(),
        summary: `Pipeline completed successfully. Generated: ${evalRes.accepted ? 'Accepted' : 'Rejected'}`
      }
    });
  } catch (dbError) {
    console.error('Error updating run status:', dbError);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { runPath, runId } = await runPipeline(goal);
